...
| 2026-09-19 | Tests that implicitly depended on untracked host artifacts (`seasid_lstm.pt`/`seasid_xgb.pkl`) passed locally but failed on the first clean CI run — `load_best` honestly serves the rules tier when no bundle exists, so `ml_bundle_loaded is True` / `forecast_source == "lstm"` assertions and an LSTM-crash path never fired. | Made model-dependent tests hermetic: `test_forecast_valid_site` asserts the contract (bool + legitimate source) instead of host state; fallback tests inject a qualifying lstm bundle via `monkeypatch.setattr(services, "load_best", …)`. The CI run itself is now the guard. | `.github/workflows/ci.yml` on every push/PR (`python -m pytest tests/`) |
| 2026-09-19 | Vitest records uncaught exceptions from event handlers and exits 1 even when every assertion passes — a new api method missing from a partial `vi.mock('@/api')` ("cancelExperiments is not a function") failed CI while a grepped local run looked green. | Component is defensive (`Promise.resolve(api.cancelExperiments?.())`) and the Experiments api mock includes the method. Check the "Unhandled Errors" section / real exit code, not just assertion counts. | `cd frontend && npx vitest run && echo EXIT:$?` and `.github/workflows/ci.yml` |
